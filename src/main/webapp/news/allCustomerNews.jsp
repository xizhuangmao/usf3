		...
		$(function(){
			...
			$.ajax({
				...
				success : function(data){
					...
					$("#sfgl_total").html(obj.total);
					$("#sfgl_totalpa").html(obj.totalPa);
					$("#sfgl_current").html(allNews_current);
					$("#sfgl_every").val(allNews_current);
					for(var i=0;i<obj.news.length;i++){
						if(obj.news[i].title.length>90){
							obj.news[i].title = obj.news[i].title.substring(0, 90) + "...";
						}
						$("#allCustomerNews_ul").append("<a style=\"cursor:pointer;color:inherit;text-decoration:none;\" href=\"newsContent.jsp?id="+obj.news[i].id+"\"><li style=\"font-size:14px;float:right\">"+obj.news[i].newsdate+"</li><li style=\"font-size:14px\">"+obj.news[i].title+"</li></a><br/>");
					}
					var tot = $("#sfgl_totalpa").html();
					if(allNews_current == tot){
						$("#sfgl_next").attr("disabled","true");
					}else{
						$("#sfgl_next").removeAttr("disabled");
					}
					if(allNews_current <= 1){
						$("#sfgl_back").attr("disabled","true");
					}else{
						$("#sfgl_back").removeAttr("disabled");
					}			
				}
			});
		
		$("#sfgl_next").click(function(){
			allNews_current += 1;
			$("#sfgl_current").html(allNews_current);
			$("#sfgl_every").val(allNews_current);
			$.ajax({
...
<div class="main" id="two-columns">
	...
	<div class="bottom" style="height: 800px;width: 850px">
		...
		<div class="left" >
			...
			<ul id="allCustomerNews_ul" class="block" style="width: 850px;border-top: 1px #BCBAAC;margin: 4px 0;padding-top: 20px;font:normal 12px/21px Arial, Helvetica, sans-serif;color:#000099;text-decoration:none;background-color:inherit;display:block;"></ul>
			
		<div class="pagin">
	    	<div class="message"><i class="blue"><label id="sfgl_total" style="display: none"></label></i><i class="blue"><label id="sfgl_current" style="display: none"></label></i></div>
	        <ul class="paginList">
	        <li class="paginItem"><button id="sfgl_back" style="cursor:pointer;"><span style="font-size: 14px;color:#000099">Previous</span></button></li>
	        <li class="paginItem" style="padding-top: 5px;display: none">第</li>
	        <li class="paginItem current" style="padding-top: 2px;">&nbsp;<input id="sfgl_every" style="width: 10px;"/></li>
	        ...
	        <li class="paginItem more" style="padding-top: 2px;"><label id="sfgl_totalpa"></label></li>
	        <li class="paginItem" style="padding-top: 5px;display: none">页</li>
	        <li class="paginItem"><button id="sfgl_next" style="cursor:pointer;"><span style="font-size: 14px;color:#000099">Next</span></button></li>
	        </ul>
    		</div>
		</div>
	</div>
